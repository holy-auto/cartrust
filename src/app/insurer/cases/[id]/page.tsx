...
export default function InsurerCaseDetailPage() {
  ...
  const fetchMessages = useCallback(async () => {
    try {
      ...
      setMessages(json.messages ?? []);
      setAttachments(json.attachments ?? []);
      // also update case status in case it changed
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      fetchMessages(); // also refreshes attachments
    } catch (e: unknown) {
      setErr(e instanceof Error ? e.message : "エラーが発生しました");
    } finally {
      setUploading(false);
    ...
    <div className="mx-auto max-w-5xl space-y-6 p-6">
      ...
      <section className="rounded-2xl border border-neutral-200 bg-white p-6">
        ...
        <dl className="grid gap-4 sm:grid-cols-2">
          <div>
            <dt className="text-xs font-medium text-neutral-500">案件番号</dt>
          ...
          <div>
            <dt className="text-xs font-medium text-neutral-500">優先度</dt>
            <dd className="mt-0.5">
              <span
                className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-semibold ${priorityClasses(caseData.priority)}`}
              >
          ...
          <div>
            <dt className="text-xs font-medium text-neutral-500">カテゴリ</dt>
            <dd className="mt-0.5 text-sm text-neutral-900">
              {caseData.category || "-"}
            </dd>
          </div>
          <div>
            <dt className="text-xs font-medium text-neutral-500">作成日</dt>
            <dd className="mt-0.5 text-sm text-neutral-900">
              {formatDateTime(caseData.created_at)}
            </dd>
          </div>
          <div className="sm:col-span-2">
            <dt className="text-xs font-medium text-neutral-500">説明</dt>
      ...
      <section className="rounded-2xl border border-neutral-200 bg-white">
        <div className="border-b border-neutral-200 px-6 py-4">
          <h2 className="text-lg font-bold text-neutral-900">メッセージ</h2>
        </div>
                ...
                >
                  <div
                    className={`max-w-[70%] rounded-xl px-4 py-3 ${
                      isInsurer ? "bg-blue-50" : "bg-neutral-50"
                    }`}
                  >
                    <span
                    ...
                    <p className="mt-1.5 text-sm text-neutral-900 whitespace-pre-wrap">
                      {msg.content}
                    </p>
      ...
      <section className="rounded-2xl border border-neutral-200 bg-white p-6">
        ...
        {attachments.length === 0 ? (
          <p className="text-sm text-neutral-400">添付ファイルはありません</p>
        ) : (
