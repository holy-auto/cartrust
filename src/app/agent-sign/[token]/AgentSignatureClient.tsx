...
/**
 * AgentSignatureClient
 *
 * 代理店向け電子署名ページのクライアントコンポーネント。
 * /sign/[token] の施工証明書版と同じ API（/api/signature/session・sign）を利用し、
 * UI テキストを代理店契約書向けに調整。
 */
...
export default function AgentSignatureClient({ token }: { token: string }) {
  ...
  if (phase === 'expired')      return <StatusScreen icon="⏰" title="有効期限切れ" message="この署名リンクの有効期限が切れています。本部へ再送を依頼してください。" />;
  if (phase === 'already_signed') return <StatusScreen icon="✅" title="署名済み"   message="この契約書はすでに署名されています。" />;
  if (phase === 'cancelled')    return <StatusScreen icon="🚫" title="無効なリンク" message="このリンクはキャンセルされています。本部へお問い合わせください。" />;
  if (phase === 'complete' && completeData) return <CompleteScreen data={completeData} />;

  // 署名フォーム
  const issuer = sessionData?.certificate?.stores?.name ?? 'Ledra';
    ...
    <div className="min-h-screen bg-gray-950 flex flex-col items-center justify-start py-8 px-4">
      ...
      <div className="w-full max-w-md mb-6">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-gray-600 text-sm">代理店ポータル</span>
        </div>
      ...
      <div className="w-full max-w-md bg-gray-900 rounded-2xl border border-gray-800 p-5 mb-4">
        ...
        <dl className="space-y-3">
          <InfoRow label="送付元" value={issuer} />
          {sessionData?.signer_name && (
            <InfoRow label="署名者" value={sessionData.signer_name} />
          )}
          {sessionData?.expires_at && (
            <InfoRow
              label="有効期限"
              value={new Date(sessionData.expires_at).toLocaleDateString('ja-JP', {
      ...
      <div className="w-full max-w-md bg-gray-900 rounded-2xl border border-gray-800 p-5 mb-4">
        ...
        <label className="flex items-start gap-3 cursor-pointer mb-6">
          <input
            ...
            className="mt-0.5 w-5 h-5 rounded border-gray-600 bg-gray-800 text-indigo-500
                       focus:ring-indigo-500 focus:ring-2 cursor-pointer shrink-0"
          />
    ...
    <div className="min-h-screen bg-gray-950 flex flex-col items-center justify-center px-4">
      <div className="w-full max-w-md text-center">
        ...
        <div className="mt-8 text-gray-600 text-sm">
          <span className="text-indigo-400 font-semibold">Ledra</span> 電子署名システム
        </div>
      </div>
    </div>
  );
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-gray-900 rounded-2xl border border-gray-800 p-5 mb-4">
          ...
          <div className="bg-gray-800 rounded-lg p-3 mb-3">
            <p className="text-indigo-300 text-xs break-all font-mono">{data.verify_url}</p>
          </div>
          <button
