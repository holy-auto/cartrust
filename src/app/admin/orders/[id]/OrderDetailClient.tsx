...
const TRANSITIONS: Record<string, Transition[]> = {
  ...
  in_progress: [{ next: "approval_pending", label: "施工完了報告", side: "to" }],
  approval_pending: [{ next: "payment_pending", label: "検収承認", side: "from" }],
};
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="glass-card p-4">
          ...
          <div className="mt-1">
            <Badge variant={STATUS_VARIANTS[order.status]}>{STATUS_LABELS[order.status] ?? order.status}</Badge>
          </div>
        </div>
        <div className="glass-card p-4">
        ...
        <div className="glass-card p-4">
          ...
          <div className="mt-1 text-sm font-semibold text-primary">
            {toTenant?.company_name ?? (order.to_tenant_id ? "—" : "未指定")}
          </div>
        </div>
        <div className="glass-card p-4">
      ...
      {counterpartyScore &&
        (() => {
          ...
          const cpCompletionRate =
            counterpartyScore.total_orders > 0
              ? Math.round((counterpartyScore.completed_orders / counterpartyScore.total_orders) * 100)
              : null;
          const cpOnTimeRate =
            counterpartyScore.completed_orders > 0
              ? Math.round((counterpartyScore.on_time_orders / counterpartyScore.completed_orders) * 100)
              : null;
                ...
                {counterpartyScore.avg_rating != null && (
                  <div className="ml-2">
                    <span className="text-yellow-500 text-sm">
                      {"★".repeat(Math.round(counterpartyScore.avg_rating))}
                      {"☆".repeat(5 - Math.round(counterpartyScore.avg_rating))}
                    </span>
              ...
              <div className="grid gap-2 grid-cols-4 text-center">
                ...
                <div className="p-2 rounded-lg bg-surface-hover">
                  <div className="text-lg font-bold text-primary">
                    {cpCompletionRate != null ? `${cpCompletionRate}%` : "—"}
                  </div>
                  <div className="text-[10px] text-muted">完了率</div>
                </div>
                <div className="p-2 rounded-lg bg-surface-hover">
                  <div className="text-lg font-bold text-primary">
                    {cpOnTimeRate != null ? `${cpOnTimeRate}%` : "—"}
                  </div>
      ...
      <section className="glass-card p-5 space-y-3">
        ...
        <div className="grid gap-3 text-sm sm:grid-cols-2">
          {order.category && (
            <div>
              <span className="text-muted">カテゴリ:</span> {order.category}
            </div>
          )}
          {order.deadline && (
            <div>
              <span className="text-muted">納期:</span> {formatDate(order.deadline)}
            </div>
          )}
          <div>
            <span className="text-muted">作成日:</span> {formatDate(order.created_at)}
          </div>
          ...
          {order.vendor_completed_at && (
            <div>
              <span className="text-muted">施工完了:</span> {formatDate(order.vendor_completed_at)}
            </div>
          )}
          {order.client_approved_at && (
            <div>
              <span className="text-muted">検収承認:</span> {formatDate(order.client_approved_at)}
            </div>
          )}
        </div>
      ...
      {order.status === "payment_pending" && (
        <section className="glass-card p-5 space-y-3">
          ...
          <div className="text-xs text-muted">
            発注者: {order.payment_confirmed_by_client ? "確認済" : "未確認"} / 受注者:{" "}
            {order.payment_confirmed_by_vendor ? "確認済" : "未確認"}
          </div>
      ...
      <section className="glass-card p-5 space-y-3">
        ...
        <div className="max-h-80 overflow-y-auto space-y-2 p-2 bg-surface-hover rounded">
          ...
          {messages.map((msg) => {
            const isMine =
              (isFrom && msg.sender_tenant_id === order.from_tenant_id) ||
