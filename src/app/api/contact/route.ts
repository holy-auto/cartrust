...
/** 送信先（問い合わせ受信用アドレス） */
const TO = process.env.CONTACT_TO_EMAIL ?? "info@linclaft.co.jp";

/** 送信元: RESEND_FROM（検証済みドメイン）を使用 */
const FROM = process.env.RESEND_FROM ?? "CAR TRUST <support@linclaft.co.jp>";
...
export async function POST(request: Request) {
  ...
  try {
    await getResend().emails.send({
      ...
      to: TO,
      replyTo: email,
      subject: `[Linclaft] お問い合わせ: ${category}（${name}）`,
      text: [
        `お名前: ${name}`,
        `メール: ${email}`,
