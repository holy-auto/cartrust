  ...
  const html = `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 560px; margin: 0 auto; padding: 24px;">
      <div style="border-bottom: 2px solid #0071e3; padding-bottom: 12px; margin-bottom: 20px;">
        <h2 style="margin: 0; color: #1d1d1f; font-size: 18px;">Linclaftへの招待</h2>
      </div>
      <p style="color: #1d1d1f; line-height: 1.6;">
        ${companyName} より、Linclaft加盟店ポータルへ招待されました。<br>
  ...
  try {
    await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from,
        to,
        subject: `【Linclaft】${companyName} から招待されました`,
        html,
      }),
    });
  } catch (e) {
...
export async function POST(req: Request) {
  try {
    ...
    const { data: insurerData } = await adminSb
      ...
      .single();
    const companyName = insurerData?.name ?? "Linclaft加盟店";
    ...
    for (const r of rows) {
      try {
        ...
        const inviteResult = await adminSb.auth.admin.inviteUserByEmail(email, {
          ...
          redirectTo: `${process.env.NEXT_PUBLIC_APP_URL ?? "https://app.linclaft.co.jp"}/insurer/reset-password`,
        });
