...
- **緩まない設計にはしてある。** max が古ければ base 比較に戻るだけ、免除欄が古ければ
  落ちるだけ。放置しても見逃しは増えず、出るのは誤検出だけ。
- しかし放置すれば、**本番へ直接当てた版が出るたびに誤検出で作業が止まる**。
  今回はそれで19時間の本番停止が伸びた。

案:
1. `db-migrate` の成功後にワークフローが `max:` を書き戻して main へ push する。
   正確で自動だが、**CI が main へ push する構成**になる（権限と無限ループの検討が要る）。
2. 免除欄が要るのは `apply_migration` を使ったときだけなので、**そちらの運用を止める**。
   穴は根本から消えるが、緊急時の手段を失う。
3. 現状維持（手動）。誤検出が出たときにエラーメッセージが更新方法を案内する。

**関連する未決**: そもそも CI から本番 DB へ問い合わせるのが最も正確だが、
CI に本番の資格情報を持たせる是非が未決。今回は「リポジトリに事実の要約を置く」で
迂回した。
