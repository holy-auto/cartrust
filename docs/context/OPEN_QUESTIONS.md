...
請求書のトークンでは 404）。ここに残すのは、それでは確かめられない2点。

**未解決1: アプリから送る動作が未確認。** Safari で確かめたのは公開ページと PDF ルートだけで、
`ReceiptShareDialog` は一度も動かしていない。要件 5.10 が求めるのは
「決済後にレシートを SMS / Email で**送れること**」で、提出ガイドもダイアログからの送信を
その動作として書いている。未確認なのは
「ダイアログの SMS / メールが実際に開くか」「本文にリンクが入るか」「共有シートが動くか」の3つ。
**ネイティブ再ビルドが要る**ので TestFlight ビルド待ち（`app.json` とネイティブ依存を触るため、
`expo start --tunnel` で JS だけ差し替えても反映されない）。審査用の録画と同時に確認する。

**未解決2: 公開レシートページの閲覧ログを残すか。** 証明書の公開ページは
`certificate_public_viewed` を監査ログに書いているが、レシート側は入れていない。
「いつ顧客が領収書を見たか」が要る場面があるかどうかで決める。**まだ何も決めていない。**

- 起票日: 2026-09-07（`#1048` で一度まとめて消しかけたのを、Codex の指摘で戻した）
