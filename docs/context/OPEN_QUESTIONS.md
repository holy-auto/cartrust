...
`origin/main` 側に元からある**（`git show origin/main:…` で2件確認済み）。

- `## M-047 マージ前の観察を根拠に、サインアップのメール確認を飛ばした（2026-09-03）`
- `## M-047 「失敗を知らせる」通知が、シェルの引用ミスで一度も飛んでいなかった（2026-09-06）`

この台帳は**番号で引くこと**が目的なので、重複は「M-047 と同じ形」という参照を
曖昧にする。ただし片方を改番すると**既存の参照がどちらを指していたか分からなくなる**ので、
こちらの判断では触っていない。

決めることは2つ:

- **どちらが M-047 を保持するか。** 先に書かれた 2026-09-03 の方を残し、
  2026-09-06 の方を末尾の空き番号へ動かすのが自然だが、既存の参照を全部当たる必要がある。
- **番号衝突そのものをどうするか。** この PR だけで**7回**衝突している
  （既存の「MISTAKE_LEDGER の連番 ID が、並行 PR のたびに衝突する（2026-09-04）」と同じ根）。
  今回のように**同じ番号のまま両方 main に入る**ところまで来ているので、
  連番をやめる（日付＋連番など）判断の材料が揃った。

## 陳腐化チェックの後始末漏れが本番で実証された。修正は #1040 で未マージ（2026-09-07）

`stale-migration-check.yml` の**初回本番実行**（2026-09-07 04:44 UTC、予定 00:20 から遅延）で、
PR #1040 で直した後始末の不具合が**実際に起きていること**を確認した。

- 3本（#1016 / #979 / #966）にコメントが付き、**3本とも本当に陳腐化していた**ので
  コメント自体は全部正しい。2本は11分以内に改番されて直った（workflow としては成功）。
- ただし**ログが汚染されている**。ループ2本目以降は前の PR のファイルが作業ツリーに残るため、
  #966 の検査結果には**自前の2件に加えて #1016 と #979 のファイル計6件**が並び、
  さらに `20260901000000` の **phantom な `duplicate-version`** まで出た。
- **今回は誤報にならなかったのは偶然。** 3本とも自前に陳腐化したファイルを持っていたため。
  1本でも綺麗な PR が混ざっていたら、**前の PR のファイルを根拠に誤ったコメントが貼られる**。
- コメント本文は固定ファイル（`.github/stale-migration-comment.md`）なのでリンタ出力を
  含まない。**だから誤った文面が人に届くことは無かった** —— これも設計というより結果論。

未解決:

- **修正（`git reset` を先に打つ）は #1040 に入っているが未マージ。** マージするまで
  毎日 00:20 UTC に汚染されたまま走る。
- 次回以降、綺麗な PR が候補に入ったときに誤報が出るかは**未観測**。

## 既に書かれた閲覧監査行に訪問者の IP が残っている（2026-09-06）

PR #1040 で、公開証明書ページが `vehicle_histories` の閲覧監査行を描画していて、
その `description` に**訪問者の IP と担当者の uid** が入っていたのを見つけた。
公開クエリで型を除外して**今後は表示されない**ようにした（DECISION_LOG 2026-09-06）。

残っている判断:

- **既存行をどうするか。** 過去の `certificate_public_viewed` / `certificate_public_pdf`
  行には IP が、`certificate_viewed` には uid が入ったまま。表示はされなくなったが
  DB には残る。監査としての価値と保持期間の折り合いを付けていない。
- **どれだけ露出していたか未計測。** いつからこの状態だったか、公開ページを持つ
  証明書が何件あるか、実際に IP を含む行が何件あるかを数えていない。
  **公開区分の判断（note に書けるか）はこの計測の後**。
- **監査ログと車両履歴を同じテーブルに置いていること自体**を分けるか。分ければ
  この種の事故は構造的に起きないが、管理画面のタイムラインが両方を混ぜて出しているので
  影響が広い。
- **他の公開経路**（公開 PDF・外部 v1 API）が同じテーブルを読んでいないかは未確認。
## PR #966 で `pull_request` 起因のワークフローが起動しないことが4回起きた（2026-09-06）

- 症状: push しても GitHub Actions が**1本も作られない**。Vercel と Supabase の
  チェックだけが付き、CI / Mobile CI / CodeQL / LoC が丸ごと出ない。
  「失敗」ではなく「存在しない」ので、PR 画面は緑にも赤にも見えない。
- 発生: `3acf7f6` / `097a043` / `20d3ce3` と、その前の1回。**毎回 main を取り込んで
  push し直すと解消する。**
- 切り分けで分かっていること:
  - main 側では同時刻に CI が正常に走っている（`6bc745f` で success）ので
    Actions の枠切れやリポジトリ全体の停止ではない
  - `097a043` のときは PR が `mergeable_state: dirty` で、マージ ref が作れず
    `pull_request` イベントが発火しない、という説明が付いた
  - **`20d3ce3` のときは main とのコンフリクトが無かった**のに同じ症状が出た。
    つまり「コンフリクトでマージ ref が作れない」だけでは説明できない
- 未解決: 真の原因。次の候補を実際に読んで潰す必要がある。
  - `.github/workflows/*.yml` の `on: pull_request` の `paths` / `branches` フィルタ
  - draft PR の扱い（`types: [opened, synchronize, reopened, ready_for_review]` の有無）
  - GitHub 側のマージ ref 計算がまだ終わっていないタイミングで push した場合の競合
- なぜ放置できないか: **「CI が緑」ではなく「CI が無い」状態は、見た目で区別しにくい。**
  マージ前に気づかなければ、検査を一度も通していない変更が入る。
  実際このブランチでは、私が毎回 Actions の一覧を数えて初めて気づいている。
- 起票日: 2026-09-06
...
## レシート公開機能のデプロイ順序と、実機での送信確認（2026-09-04）

**順序の制約（確定・判断待ちではない。忘れると事故る）**:
モバイルの新ビルドは `/receipt/<public_id>` のリンクを送る。**Web を先にデプロイしないと、
そのリンクは 404 のまま**（ページも PDF ルートも存在しない）。順序は

1. ~~マイグレーション適用（`documents.public_id` ＋ 索引）~~ → **2026-09-06 適用済み**
   （`20260906094512` / `20260906094735`）。実機確認を先に回すため、Web デプロイより前に当てた
2. Web デプロイ（`/receipt/[public_id]` と `/api/receipt/pdf`）← **PR #966 のマージ待ち**
3. モバイルのビルド提出

逆順にすると、直したはずの 404 がそのまま残る。2026-08-25 に同種の順序制約を起票済み。

**未解決（一部前進）**: DB を当てたので、**Vercel プレビュー URL なら iPhone で
「リンクを開く → 領収書が出る → PDF が落ちる」までを今すぐ確認できる**（代表に手順を渡した）。
残るのは**アプリ側から送る**部分で、こちらはネイティブ再ビルド（TestFlight か dev-client）が要る。
`expo start --tunnel` で JS だけ差し替えても、この PR は `app.json` とネイティブ依存を触るので反映されない。

**未解決**: **実機（iPhone）での送信確認が未実施。** 要件 5.10 は「送れること」なので
公開ページ＋PDF で満たす想定だが、確認できていないのは
「SMS / メールアプリが実際に開くか」「送ったリンクを iPhone の Safari で開けるか」
「PDF がその場で表示・保存できるか」の3つ。Apple 審査用の録画を撮るときに合わせて見る。

**未解決（小）**: 公開ページの閲覧ログを残すか。証明書の公開ページは
`certificate_public_viewed` を監査ログに書いているが、レシート側は今回入れていない。
「いつ顧客が領収書を見たか」が要る場面があるかどうかで決める。
...
- 選択肢: 案A `pod install` を CI（macOS ランナー）で回して整合を見る。確実だが macOS ランナーは
  ...
  案C 現状維持。iOS は `development-device` で継続的に実機ビルドしており、
  Android と違って「一度も通っていない」状態にはならないので、そもそも露出が小さい。
- 影響範囲: 誤ると（＝案Cのまま事故ると）iOS のリリースビルドが直前に落ちる。
  ただし iOS は実機検証で日常的にビルドしているため、Android のように長期間潜伏はしにくい。
- 次のアクション: 新しいネイティブ依存を iOS に足すときに案Bを検討する。それまでは案C。
- 起票日: 2026-08-25

## 起動オープニングで毎回1.85秒待たせることを、現場スタッフが許容するか（2026-08-23）
- 状況: モバイルアプリのコールドスタートに 2.0 秒のロゴスティングを入れた。退場条件は
  「起動処理が終わり、かつ 1500ms 経過」＋350msフェードなので、**下限が約1.85秒**になる。
  現状の起動処理は `useAuthInit` の実測で未ログイン約300ms、ログイン済み300ms〜1.5秒。
  未ログイン起動にはこの1.85秒がほぼ丸ごと上乗せになる。ログイン済みなら大半が隠れる。
  出張整備の現場でアプリを1日に何度も開き閉めする使い方だと、この差が効いてくる可能性がある。
- 選択肢:
  - 案A 現状維持（1.85秒）: ブランド体験は最大。日常の起動は毎回1.5秒前後遅くなる。
  - 案B `INTRO_MIN_MS` を下げる: 定数1つで調整できる。ただしロックアップが完成するのが
    実測1.2秒なので、1200ms を下回ると LEDRA が出きる前に消える（自己チェックが弾く）。
    実質1.2〜1.5秒の幅しか無い。
  - 案C 素材側を詰める: ロックアップ完成までの1.2秒を短くしてもらう。下限を根本的に下げられる
    唯一の手段だが、素材の作り直しが要る。
  - 案D 初回起動のみ再生: 日常の起動を一切遅くしない。2回目以降はブランド体験がゼロになる。
- 影響範囲: 誤ると、毎日使う道具の体感速度を恒久的に損なう。逆に外しすぎると、
  未リリースのアプリの第一印象を作る唯一の場面を捨てることになる。
  `INTRO_MIN_MS` は定数1つなので、後から変えるコスト自体は小さい。
- 補足（2026-08-23 追記）: 演出の直後に毎回入っていた2度目のちらつき
  （`/(tabs)` → `/(auth)/select-store` → `/(tabs)` の往復）は解消済み。
  店舗の解決を起動処理に前倒ししたため、待ち時間は変わらないが「勝手に画面が2回変わる」
  挙動は無くなった。実機で判断する際は、1.85秒の待ちだけを評価すればよい。
- 次のアクション: 代表が dev-client ビルドを実機に入れ、実際の現場の使い方で数回起動して判断する。
- 起票日: 2026-08-23
...
## 【解決済み 2026-09-06】マイグレーション外で本番へ入ったオブジェクト 63 個（2026-09-04 起票）
...
動的 SQL で作られるトリガ 6 本を「未管理」と誤検出していた。MISTAKE_LEDGER M-061）
...
| テーブル | 278 | **23** |
| ビュー | 4 | **1** |
| 関数 | 141 種（名前で数えた。拡張機能所有ぶんは除外） | **24** |
| トリガ | 129 | ~~15~~ → **9**（6 本は誤検出。下記参照） |
| enum 型 | 5 | **5**（migrations に `CREATE TYPE` が1本も無い） |
| イベントトリガ | 1 | **1**（`ensure_rls`。棚卸しの翌日に追加で発見） |
...
### 【解決済み 2026-09-06】63 個すべてに処遇を付けた

同日中に全件を片付けた（DECISION_LOG 2026-09-06「ドリフト 63 個を解消し…」）。
数え直しの経緯: 棚卸しの 68 に、見落としていたイベントトリガ `ensure_rls`
（新規テーブルに RLS を自動で有効化する）を足して 69 → **そこから、試作の
検出器が誤検出していたトリガ 6 本を引いて 63**。
正しい内訳はテーブル23・ビュー1・関数24・トリガ9・enum型5・イベントトリガ1。

- **削除**: テーブル 23・関数 7（うち 1 つは呼び出し不能な overload）・
  そこに付いていたトリガ 6・`job_orders` の越境ポリシー 1・
  `storage.objects` の不活性ポリシー 2
- **書き起こし**: 関数 17・トリガ 3・ビュー 1・enum 型 5・イベントトリガ 1
- **検出器**: `scripts/check-schema-drift.mjs` を `supabase-advisors.yml`（週次）に追加

検証: 空 DB へ全マイグレーションを流した DB と、削除後の本番を突き合わせ、
**テーブル・ビュー・関数・トリガ・enum・イベントトリガのすべてで「本番にあって
再生側に無いもの」が 0 件**になることを確認した。

**当初の分類は一部間違っていた**（MISTAKE_LEDGER M-060）。「23 本はアプリからの参照
...
さらに 1 本は **`storage.objects` のポリシー 2 本**から使われていた。
...
- **いつ・誰が・なぜ作ったか**は依然不明【要確認】。Postgres はオブジェクトの
  ...
  検出器を入れたので、**これ以降に増えた分は週次で分かる**。過去の経緯は追えない。

## 本番と migrations で列の型が違う。再生した DB に本番データを入れると弾かれる（2026-09-06）

ドリフト棚卸しの副産物。**オブジェクトの有無より深い差**が見つかった。

本番の中核テーブルは enum 型の列を持つが、その列を作るマイグレーションは
`text + check` で宣言している。

| 列 | 本番 | migrations |
|---|---|---|
| `tenants.plan_tier` | `plan_tier_enum`（mini/standard/pro/**free**/**starter**） | `text check in ('mini','standard','pro')` |
| `certificates.status` | `certificate_status_enum` | `text default 'active'` |
| `certificates.expiry_type` | `expiry_type_enum` | `text` |
| `tenant_memberships.role` | `membership_role_enum` | `text` |
| `templates.scope` | `template_scope_enum` | `text` |

**`plan_tier` は値の集合まで違う。** 本番の 24 テナントのうち **20 件**（free 19 /
starter 1）が、マイグレーション側の check に**弾かれる**。つまり
