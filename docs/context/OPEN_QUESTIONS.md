...
### 【解決済み 2026-09-06】69 個すべてに処遇を付けた

棚卸しの翌日に全件を片付けた（DECISION_LOG 2026-09-06「ドリフト 69 個を解消し…」）。
**追加で 1 個見つかったので 68 → 69**。イベントトリガ `ensure_rls`（新規テーブルに
RLS を自動で有効化する）もマイグレーションに `CREATE EVENT TRIGGER` が無かった。

- **削除**: テーブル 23・関数 7（うち 1 つは呼び出し不能な overload）・
  そこに付いていたトリガ 6・`job_orders` の越境ポリシー 1・
  `storage.objects` の不活性ポリシー 2
- **書き起こし**: 関数 17・トリガ 9・ビュー 1・enum 型 5・イベントトリガ 1
