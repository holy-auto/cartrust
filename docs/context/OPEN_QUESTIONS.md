...
- **PR #979 への push 2回（`f97fbe0` / `2c78c1d`、8/27 00:10〜00:12 UTC）で
  GitHub Actions が1本も起動しなかった【要確認】。** 同時刻帯に他 PR の CI は
  正常に走っており（00:09〜00:40）、このブランチだけ `pull_request` の run が
  作られていない。ワークフロー側に `concurrency` の設定は無いので、まとめて
  キャンセルされたわけでもない。head は Vercel と Supabase のチェックだけが付いた状態だった。
- main を取り込んだ次の push（`1804721`、00:52 UTC）では正常に起動し、全ジョブ緑。
  **原因不明のまま解消**しているので、再発したら「main を取り込んで押し直す」で
  逃げられることだけは分かっている。GitHub 側の一時障害か、ブランチ単位の
  スロットリングかは切り分けられていない。

## 追加（2026-08-26・Square 経由の QR コード決済）

- **Square API を1回も叩いていない【要確認】。** この環境から
  `developer.squareup.com` にも本番キーにも届かない。Sandbox（日本ロケーション）で
  (1) デバイスコードの発行とペアリング、(2) `payment_type: "QR_CODE"` の
  Terminal チェックアウト、(3) POS アプリ会計の引き当て、を1回ずつ通す必要がある。
- **Square 端末を買うか。** 「店員が Ledra から移動しない」を成立させる条件が
  端末なので、実質これが値段。買わない場合は Square アプリへの移動が残る。→ 代表判断
- **引き当ての窓（直近30分・同額・同店舗）が実運用で足りるか。** 同額の会計が
  重なると「特定できない」として記帳しない設計にしてある（取り違えより未記帳を選ぶ）。
- **既存の Square 接続は繋ぎ直しが必要。** OAuth 権限を追加したため、古いトークンでは
  決済を起こせない（403 になる）。加盟店への案内が要る。
- **モバイルアプリ側の導線が未実装。** サーバの API は共通なので、次のリリースで
  載せられる。

## 追加（2026-08-26・決済手段の拡張）

- **Alipay / WeChat Pay が日本の加盟店で出せるか未検証。** 出せなければ探りで
  落ちて自動的に外れる（会計は止まらない）が、「使える」と案内する前に
  1店舗で確認が要る。インバウンド客のいる店だけの話。
- **PayPal は日本の加盟店で使えるか未確認。** SDK の union にはある。
- **レジ締めで PayPay / Alipay / WeChat Pay がすべて「QR決済」に丸められる。**
  内訳が要るか。→ 代表判断
- **d払い・au PAY・楽天ペイ・メルペイは Stripe が対応していない。** 使うなら
  各社と直接契約して店舗の端末で決済し、Ledra には記帳だけする形になる。
  他社 PSP を足すかは事業判断。→ 代表判断
...
- ~~**PayPay を要求すると、使わない店にもオンボーディングの入力が増える可能性。**~~
  → **2026-08-26 決着**: 代表方針「Ledra 側としては強制しない」。選択制にした
  （既定は要求しない）。DECISION_LOG 2026-08-26 参照。
...
- **Stripe API が実際に `paypay` を受けるか未検証。** PayPay は public preview
  （clover 2025-09-30）で、SDK v20.4.1 の型 union にも入っていない。実装は
  「拒否されたらカードのみで作り直す」ので落ちはしないが、**対外的に
  「PayPay が使える」と言う前に本番のテストキーで1回確かめる必要がある。**
  → 代表が Stripe ダッシュボードで PayPay を有効化 → 実機で QR を出す、で確認可能。
  そのとき**同じテストで「PayPay で払った直後にレジが完了になるか」も見る**
  （レジは `payment_status === "paid"` を5分ポーリングする。PayPay が
  コンビニ払いのような後日確定だと、払われたのに売上が立たない）
- **PayPay の申請にかかる実際の期間【要確認】。** Stripe の記載は「数週間」。
  加盟店に案内する数字なので、1店舗通してから言う。→ 代表判断
- **加盟店に PayPay を勧めるか。** Stripe 経由の手数料と PayPay 直契約の
  比較をしていない。→ 代表判断
- **POS の「QR決済」が2つの意味を持つことになった。** Stripe 経由の PayPay
  （自動で `qr` として記帳）と、他社端末で受けた決済の手入力記録が同じラベル。
  レジ締めで区別が要るか。→ 代表判断
- **モバイルアプリの案内文はカード前提のまま。** サーバ側は PayPay を出すが、
  アプリの文言更新は次のリリースに乗せる必要がある。
