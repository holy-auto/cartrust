...
> （予約経路にも最初から入っていたので、Apple 要件5.10は実質未達）。`/receipt/<token>` の
> 公開ページと PDF を追加し、iPhone で「開く・PDFを保存する・請求書のトークンでは404になる」
> の4項目を確認済み。**doc_type='receipt' 以外は本番でも出ない**ことを実物で確定させた。
> あわせて、止まっていた本番のマイグレーション自動適用も復旧した（#1042 と本PRの両方が要った）。
> **iPhone 単独ローンチの残作業は、審査用の録画と publishing entitlement の2つ。**
> アプリから実際に送る動作の確認は TestFlight ビルド待ち。
