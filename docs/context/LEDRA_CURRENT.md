...
> **確認できたのは「送られたリンクの側」まで。** `ReceiptShareDialog` から実際に
> SMS / メールが飛ぶかは未確認で、ネイティブ再ビルドが要るため TestFlight ビルド待ち
> （OPEN_QUESTIONS に起票）。**iPhone 単独ローンチの残作業は、アプリから送る動作の確認・
> 審査用の録画・publishing entitlement の3つ。**
