...
> 2026-09-07 追記: **本番へのマイグレーション自動適用が19時間止まっていた**（#1020 の
> マージ `6bc745f7` 2026-09-06 12:57 UTC 〜 2026-09-07 14:56 UTC）。**現在は復旧済み**
> （`DB migrate` run #64 成功。本番 448版 = repo 448ファイル、#1020 の4本も適用済み）。
> 原因は2つで、#1042（`b82e2a05`）が out-of-order（不変条件2）を、
> **#966 のマージ（`8c0c2576`）が不変条件1**を解消した。
> 不変条件1は `20260906094512` / `20260906094735` が本番の `schema_migrations` に
> あるのにファイルが main に無い状態で、**#966 の head から2ファイルをバイト単位で
> コピーして補った**（当初は #966 のマージを待つ判断だったが、#966 がドラフトのまま
> 動かないため方針を変えた。内容が同一なので #966 側は壊れない）。
> **この過程で、前日に入れたガードの前提が偽だと判明した** ——
> 「base（main）の最新 >= 本番の最新」は `apply_migration` で本番へ直接当てた版が
> あると成り立たない。out-of-order 検査の比較対象を本番台帳の要約
> （`supabase/migrations.production-ledger`）へ移し、#1020 当時の値を回帰テストにした
> （DECISION_LOG 2026-09-07、MISTAKE_LEDGER M-061）。
> 同じ形の停止は 2026-08-02〜08-15 に13日間続き、証明書発行が全件停止している。
> **残: `npm run db:typegen`**（#1020 の4本が本番へ適用されたため。この環境は DB 未接続）。
> 失敗の Slack 通知は #1042 まで**一度も飛んでいなかった**（通知ステップ自身がシェルの
> 引用ミスで毎回落ちていた）。2026-09-06 15:34 UTC の run #63 で初めて配信された。
> 詳細は RELEASE_LOG 2026-09-06、MISTAKE_LEDGER M-045 / M-060。
