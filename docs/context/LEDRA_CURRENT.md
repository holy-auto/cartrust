...
> マージ `6bc745f7` 2026-09-06 12:57 UTC 〜）。原因は2つで、#1042（`b82e2a05`）が
> out-of-order（不変条件2）を、#1044 が不変条件1を直した。
...
> （DECISION_LOG 2026-09-07、MISTAKE_LEDGER M-048）。
> 同じ形の停止は 2026-08-02〜08-15 に13日間続き、証明書発行が全件停止している。
> **#1044 マージ後に `DB migrate` が緑に戻るかの確認と `npm run db:typegen` が要る。**
> 失敗の Slack 通知は #1042 まで**一度も飛んでいなかった**（通知ステップ自身がシェルの
> 引用ミスで毎回落ちていた）。2026-09-06 15:34 UTC の run #63 で初めて配信された。
> 詳細は RELEASE_LOG 2026-09-06、MISTAKE_LEDGER M-045 / M-047。
