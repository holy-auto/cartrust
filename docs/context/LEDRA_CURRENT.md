...
> ダッシュボードで申請できる。申請と確認の手順は `docs/payment-methods-setup.md`
> にまとめた（加盟店向け／代表の検証手順の2部構成）。

> 2026-08-26 追記9: **Stripe で出せない QR 決済（d払い / 楽天ペイ / au PAY /
> メルペイ）を Square 経由で足した。** Square は日本で主要7種に対応し、申請1回で
> 全ブランドが使える。**Square 端末があれば Ledra から移動せず**に会計できる
> （Terminal API がマルチブランド QR を端末に出す）。端末が無い場合は Square の
> 仕様上アプリへの移動が要るので、会計後に Ledra が実績を引き当てて記帳する。
> 記帳の二重計上は `payments.square_payment_id` の一意インデックスで止める。
> **Square API は未検証**（Sandbox で1回通す必要がある）。モバイル導線は未実装。
...
> 2026-08-26 追記8: **店頭 QR 会計で使える決済手段を全部載せた。** カードに
> 加えて **PayPay / Alipay / WeChat Pay**（いずれも即時確定）を提示し、その店で
> 有効化されていない手段**だけ**を外す。コンビニ払い・銀行振込は請求書の決済
> リンク側に任せる（レジに出すと入金確定が後日で売上が立たない）。Connect 作成
> 時の同時申請も4つ（PayPay / コンビニ / 銀行振込 / Link）に拡張。
> **d払い・au PAY・楽天ペイ・メルペイは Stripe が対応しておらず出せない。**
...
> 2026-08-26 追記7: **PayPay の利用申請を Connect オンボーディングと同時に
> 出すようにした。** `accounts.create` で `capabilities.paypay_payments` を要求
> するので、Stripe のオンボーディングが PayPay に必要な情報も同じ入力フローで
> 集める（加盟店の手続きが1回で済む）。要求が通らない環境では PayPay 抜きで
> 作り直すため、接続そのものは止まらない。**効くのは新規接続だけ**で、既に
> 接続済みの加盟店は Stripe ダッシュボードから自分で申請する必要がある。
...
> 2026-08-26 追記6: **店頭 QR 会計に PayPay を出せるようにした。** これまで
> QR 会計は `payment_method_types: ["card"]` 固定＝「QR を見せてカードで払って
> もらう」もので、POS の「QR決済」は Stripe を通らない**記録だけ**だった。
> 今は `["card", "paypay"]` で Session を作り、**Stripe に断られたらカードのみで
> 作り直す**（`src/lib/stripe/posCheckoutSession.ts`）。PayPay は Stripe 側で
> 店舗ごとの申請・審査が要るため、有効化した店だけ自動で使えるようになる。
> 記帳する会計手段も Stripe の実績から決め、PayPay は「QR決済」として残す。
> **未検証**: 本番キーが無く、API が実際に `paypay` を受けるかは未確認
> （受けなければ従来通りカードのみで動く）。
