...
| 型 | 中身 | 該当 |
|---|---|---|
| **A. 道具を検証しない** | 変更したコードは検証したのに、「何を変えるべきか判断するために使った道具」（検出器・正規表現・スクリプト・クエリ）を検証していない。道具の出力を事実として扱う。**そのクエリが何を確定させたのかを確かめずに、確定したつもりになる**のもこの型。**自分の想定だけで検証するのも同じ**（当たりを取る「既知の1件」を自分で作る） | M-001, M-002, M-003, M-006, **M-010**, M-012, M-016, M-017, **M-022**, **M-024**, **M-031**, M-036, **M-041**, **M-044**, **M-046**, **M-048** |
...
## M-048 ガードの根拠として書いた「十分条件」が、そもそも成り立っていなかった（2026-09-07）
...
**再発防止**:
- 回帰テスト5件を `scripts/__tests__/lint-migrations.test.ts` に追加。うち1件は
  **#1020 の実際の値**をそのまま使う。この検査を将来どういじっても、#1020 は再び落ちる。
- 台帳ファイルは古くなっても**緩まない**設計にした（max が古ければ base 比較に戻るだけ、
  免除欄が古ければ落ちるだけ）。壊れた行は黙って捨てず例外にする。
- **「十分条件」「〜なので安全」と書きたくなったら、その根拠が依存している別の前提を
