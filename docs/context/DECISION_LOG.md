...
## 2026-08-26 本番の台帳に書く経路が2つあった —— `db-migrate` と Supabase の GitHub 連携
1. 日付: 2026-08-26
2. 起きたこと: 代表の「調べて修正して」を受けて、`20260825000000` が誰に適用されたのかを調べた。**Supabase の GitHub 連携 (Branching) だった。** `list_branches` で本番プロジェクト `cahybswpduchptvyvdkk` に既定ブランチ `main` が紐づいており (`is_default: true`・`project_ref` が親と同一)、`main` への push で `supabase/migrations/**` を本番へ適用している。postgres_logs に決定的な証拠があった。
3. 以前の考え: 本番の台帳に書くのは `db-migrate`（`supabase db push --db-url`）だけだと思っていた。だから「`db-migrate` が『未適用』と言って落ちた」なら、そのファイルは本番に無いと読んでよいはずだった。
4. 違和感・問題: (a) **その前提が誤りだった。** 実測（postgres_logs・2026-08-26）: `08:23:34` に `db-migrate` が out-of-order で exit 1（このとき `supabase migration list` は `20260825000000` を「未適用」と表示）。その **22秒後の `08:23:56`**、別のクライアントが `CREATE TABLE IF NOT EXISTS supabase_migrations.schema_migrations` から始めて同じファイルの6文を実行し、台帳へ入れた。`schema_migrations.created_by` は null（CLI 経路。ダッシュボード/MCP 経由なら `info@holy-inc.jp` が入る）。GitHub Actions 側にこの実行のログは無い。(b) この経路は**順序を見ない**。`db-migrate` が拒否した out-of-order をそのまま当てる。(c) 結果、`db-migrate` の失敗ログを根拠に「未適用だから改名してよい」と判断すると、22秒後には適用済みになっていて条件1（本番にあるバージョンのファイルが repo に必要）を壊す。**私はこれを踏んだ。** (d) 直近の停止 #971・#972・#973 と、今日の2回は、いずれもこの二重書き込みの症状として説明がつく。
5. 決めたこと: (a) `db-migrate.yml` の不変条件コメントに**経路が2つある事実と22秒の実測**を書き、「(a) の失敗ログを根拠に改名しない。失敗した後は必ず台帳を引き直す」を明記した。(b) **恒久対策は「書き手を1つに絞る」** —— Supabase ダッシュボードの Integrations で本番への自動適用を切り、`db-migrate` を唯一の書き手にする。ただしこれは**本番プロジェクトの設定変更**で、開いている PR #938 / #941 のプレビューブランチにも影響するため、代表判断を待つ（未実施）。
6. 捨てた選択肢: (a) `db-migrate.yml` を削除して Supabase 連携を唯一の書き手にする＝順序の強制も Slack 通知も失う。13日間気づかれなかった前例（DECISION_LOG 2026-07-21）に逆戻りする。(b) `--include-all` で強制適用＝順序の不変性が崩れる。2026-07-21 で不採用済み。(c) 私の判断で既定ブランチを削除して Branching を切る＝本番設定の変更であり、開いている PR のプレビュー環境も壊す。代表の許可なくやらない。(d) `lint-migrations` に本番との突き合わせを実装＝CI から本番DBの認証情報が要るうえ、**そもそも検出の問題ではない**。検出はできていた（`db-migrate` は正しく落ちていた）。壊れているのは書き手が2つあることの方。
7. 判断理由: 「症状でなく根因」。#971 から今日まで5回、私（と前のセッション）はファイル名を直し続けていた。1件ずつは正しい対処だったが、**同じ症状が5回出るなら直しているのは症状の方**。実測で根因に到達したのは、失敗ログを信じるのをやめて postgres_logs を直接読んだとき。ログは「何が起きたか」を持っていて、Actions のログは「片方の経路で何が起きたか」しか持っていなかった。
8. まだ答えが出ていないこと: (a) **Supabase 連携の本番自動適用を切るか【代表判断】。** 切れば書き手が1つになるが、Branching のプレビュー環境も止まる（現在 PR #938・#941 が使用中。どちらも `MIGRATIONS_FAILED` 状態で、同時プレビューブランチ上限に達しているため**全 PR で `Supabase Preview` が cancelled になっている**）。(b) 切らない場合、この二重書き込みは残り続ける。コメントで注意を促すだけになる。(c) 過去の停止（#971・#972・#973）が同じ原因だったかは状況証拠のみで、当時の postgres_logs は保持期間外の可能性がある【要確認】。
9. 公開区分: 公開可（「同じ症状を5回直したら、直しているのは症状の方」「自動化の経路が2つあると、片方のログが嘘になる」「失敗ログは『適用されなかった』を意味しない」は発信可。プロジェクト ID・接続情報・メールアドレスは出さない）
