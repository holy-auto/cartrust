...
## 2026-09-07 out-of-order 検査の比較対象を、main の代用から本番台帳の要約へ移す

1. **日付**: 2026-09-07（`date -u` で確認、07:53 UTC）
2. **起きたこと**: #966 のマージ待ちで本番の適用が19時間止まっていた（#1020 のマージ
   2026-09-06 12:57 UTC から）。#966 はドラフトのまま動かず、main と競合し、51ファイル・
   27コミットある。待つ間、スキーマ変更は本番へ一切届かない。
   そこで #966 の2ファイル（`documents.public_id` とその部分ユニーク索引）を head から
   **バイト単位でコピー**して main へ補おうとしたところ、**自分が前日に入れた検査
   `migration-version-before-base-head` が落とした。**
3. **以前の考え**: 「base（main）に在るどのファイルよりも後のバージョンなら
   out-of-order にならない」を十分条件として、本番へ問い合わせずに判定できることを
   利点と考えていた。コードのコメントにも根拠としてそう書いていた。
4. **違和感・問題**: その前提 `base の最新 >= 本番の最新` が**偽**だった。
   `apply_migration` で本番へ直接当てた版は main を通らない。実際 #966 がそうしていた。
   検査は2方向に外れていた —— #1020 では代用値が低すぎて**見逃し**（本番停止の原因）、
   今回は高すぎて**誤検出**。**同じ 2026-09-06 に「マイグレーション外で本番へ入った
   オブジェクトが 68 個ある」と自分で棚卸ししながら、それが起きない前提のガードを
   書いていた**（M-061）。
5. **決めたこと**: `supabase/migrations.production-ledger` に本番台帳の要約
   （最新版1つ＋main を通っていない適用済み版の一覧）を置き、しきい値を
   **base の最新と本番の最新の大きい方**にする。本番が適用済みの版は免除する。
   あわせて #966 の2ファイルを main へ補い、不変条件1を解消する。
6. **捨てた選択肢**:
   - **#966 のマージを待つ** — 本番停止が続く。前回の同型事故は13日続き証明書発行が
     全件停止した。#966 は8時間半動いておらず、いつ入るか読めない。
   - **CI から本番 DB へ直接問い合わせる** — 最も正確だが、CI に本番の資格情報を
     持たせる是非が OPEN_QUESTIONS で未決。ネットワーク断で検査が落ちる脆さも増える。
   - **該当ルールを一時的に無効化する / ファイル単位の免除を作る** — 代表の指示は
     「移植するが検査は緩めない」。汎用の逃げ道は、未適用の古い版を通す穴になる。
   - **`supabase migration repair --status reverted`** で本番台帳から2版を消す —
     不変条件1は消えるが、#966 がマージされた瞬間に同じ2版が main の最新より古い
     未適用として現れ、out-of-order で再び止まる。問題を後ろへ送るだけ。
   - **全400版をファイルに写す** — 手で写した誤記が、それ自体「判断の道具」の欠陥になる。
     検査に要る事実は2つだけなので2つに絞った。
7. **判断理由**: これは検査を**緩める**変更ではなく、代用を実物に寄せて**強くする**変更。
   証拠として、#1020 当時の値（base 20260905142740 / 本番 20260906094735 /
   追加 20260906000000）をそのまま回帰テストにし、**しきい値を base だけに戻す変異で
   そのテストだけが落ちること**を確認した。つまり新しい検査は #1020 を止められる。
   2ファイルは #966 と内容が同一なので、#966 のマージ時に git が静かに解決する。
   本番へ DDL は流れない（`db push` は適用済み版を再実行しない）。
8. **まだ答えが出ていないこと**:
   - **【この PR 内で訂正】** 当初「台帳が古くなっても緩まない（max が古ければ base 比較に
     戻るだけ）」と書き、同じ主張をこの欄を含む**4つの文書に載せた**。**これは偽だった。**
     base 比較に戻るということは **#1020 を通した当時の検査に戻る**ということで、
     見逃しそのものである。再現して確認した（`/code-review` の指摘、M-061）。
     対処として `db-migrate.yml` に「本番が repo より先に進んでいるのに台帳に記録が
     無い」場合にジョブを赤くするステップを足した。**本番への適用は止めない**（台帳の
     古さで適用を止めては本末転倒なので、push の後に置いた）。
   - **残る天井**: `apply_migration` で直接当てた直後から、次に `db-migrate` が走るまでの
     窓は誰も見ていない。定期実行にするか、直接適用の運用をやめるかが要る。
   - **台帳を誰がいつ更新するか**は依然として手動。上の検査は「古い」と言うだけで直さない。
   - #966 がこの2ファイルを編集した場合、マージ時に実コンフリクトになる。現時点では同一。
   - **#966 は同じルールに汎用の逃げ道（`allowlist.has(file)`）を足している。**
     この PR はそれを意図的に採らなかった。コンフリクト解消時に #966 側を取ってはいけない。
9. **公開区分**: 公開可（検査の設計判断。本番の識別子・資格情報・顧客データを含まない）
