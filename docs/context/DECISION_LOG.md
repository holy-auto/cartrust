...
7. **判断理由**: これは検査を**緩める**変更ではなく、代用を実物に寄せて**強くする**変更。
   ...
   台帳が古くなっても緩まない（max が古ければ base 比較に戻るだけ、免除欄が古ければ
   落ちるだけ）ので、放置しても見逃しは増えない。
   2ファイルは #966 と内容が同一なので、#966 のマージ時に git が静かに解決する。
   本番へ DDL は流れない（`db push` は適用済み版を再実行しない）。
8. **まだ答えが出ていないこと**:
   - **台帳ファイルを誰がいつ更新するか。** 今は手動。`db-migrate` の成功後に
     自動で書き戻す案があるが、ワークフローが main へ push する構成になる。OPEN_QUESTIONS へ。
   - `apply_migration` で本番へ直接当てる運用自体を止めるか。止めればこの穴は消えるが、
     緊急時の手段を失う。
