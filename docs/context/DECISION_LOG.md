...
## 2026-09-06 ドリフト 63 個を解消し、検出器を週次に置いた。「使われていない」は DB の内側まで見ないと言えない

1. **日付**: 2026-09-06
2. **起きたこと**: 同日午前に棚卸しだけして保留した 3 件（検出器・テーブル削除・関数の
   書き起こし）を、代表の「全部」の指示で一度に片付けた。作業中に**棚卸しの分類が
   一部間違っていた**ことと、**オブジェクトの有無より深いドリフト**が判明した。
3. **以前の考え**: 「23 本はアプリからの参照ゼロ・合計 2 行だから無害。消すのは簡単」。
   関数は「本番の定義をそのまま書き写せば済む」。
4. **違和感・問題**: 削除の前に `pg_depend` で依存を洗ったら、**3 つ外れていた**。
   - **`dealers` / `dealer_users` はアプリからは参照ゼロでも、DB の内側から使われていた。**
     関数 5 本が読み、その関数を 26 本の RLS ポリシーが使う。うち
     **`market_is_approved_dealer()` は `storage.objects` のポリシー 2 本**から
     使われていた —— 削除対象外の、実在するテーブルである。
     「アプリのコードを grep してゼロ」を「使われていない」と読んでいた（M-060）。
   - **`search_vehicles_for_cartrust` の 3 引数版は、本番でも呼び出し不能**だった。
     4 引数版の `p_status` に既定値があるため 3 引数の呼び出しが必ず曖昧になる
     （`is not unique`）。実際に本番で呼んで確認した。
   - **列の型が本番と migrations で違う。** 本番の `tenants.plan_tier` は enum
     （5値）だが、マイグレーションは `text check in ('mini','standard','pro')`。
     **本番 24 テナント中 20 件がこの check に弾かれる。**
   さらに、書き起こしを CI にかけたらリポジトリ自身の検査が 2 回止めた。
   再生検査が戻り値型の不一致を、`lint:migrations` が SECURITY DEFINER 5 本の
   可変 `search_path` を捕まえた。
5. **決めたこと**:
   - **削除**は依存を pg_depend で洗ってから。テーブル 23・関数 7・トリガ 6・
     ポリシー 3（`job_orders` の越境 1 と `storage.objects` の不活性 2）。
   - **書き起こし**は「定義」だけでなく**実行権限まで**写す。`current_insurer_id` と
     `rls_auto_enable` は本番で PUBLIC 実行権が剥がしてあり、定義だけ写すと
     **再生した DB のほうが緩くなる**。
   - `lint:migrations` の指摘は **allowlist に逃げず直す**。5 本とも本体は
     スキーマ修飾済みだったので `search_path = ''` にして、再生 DB で実際に
     呼んで動くことを確かめた。
   - **検出器は「マイグレーションの字面を読む」のではなく「実際に作った DB を見る」。**
     `replay-migrations.mjs --dump` の出力と本番を突き合わせる。
   - **列の型は今回やらない。** 別項として OPEN_QUESTIONS に起票。
6. **捨てた選択肢**:
   - **正規表現でマイグレーションを読む検出器**（棚卸しで使った試作）→ 動的 SQL
     （`execute format('create ...')`）を取りこぼす。現に書き起こしのトリガを
     最初 DO ブロックで書いており、**自分の検出器から見えない状態**を作りかけた。
     リテラルに書き直したうえで、検出器も pg_dump を読む方式に替えた。
   - **`Migrations Replay`（PR の CI）に本番との突き合わせを足す** → あのジョブは
     空 DB しか持たない。PR ジョブへ本番トークンを渡す代償が検出の価値に見合わない。
   - **`dealers` / `dealer_users` を残す** → 残すと関数 5 本と 26 ポリシーも残り、
     「空の表を見て常に false を返す認可関数」が残る。使われていない機能の
     認可経路は、消すほうが安全。
   - **3 引数 overload も書き起こす** → 呼べない関数を正式化することになる。
7. **判断理由**: 「使われていない」の判定には**アプリ・RLS ポリシー・関数本体・
   外部キー・他スキーマ（storage）**の 5 経路がある。今回はアプリだけ見て
   結論を出しかけた。**消す前に pg_depend を引く**のが、この 5 経路をまとめて
   見る唯一の安い手段だった。
   検出器を字面ではなく実物にしたのも同じ理由で、**字面は書き方次第で嘘をつく**。
8. **まだ答えが出ていないこと**: 列の型のドリフト（`plan_tier` ほか 4 列）をどう直すか。
   `alter column ... type` を当てるか、check を広げるか。
   上記 5 列以外に型の食い違いが無いかも未調査【要確認】。
   ドリフトが**いつ・誰の手で**入ったかは依然追えない（Postgres に作成時刻が無い）。
   検出器はこれ以降の増加しか見られない。
9. **公開区分**: 公開可（テーブル名・関数名・件数のみ。データの中身・個人情報を含まない）
