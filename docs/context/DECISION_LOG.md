...
## 2026-09-01 保険会社ポータルの検索が本番で壊れていた（search_path を締めたとき本体の修飾を忘れた2本）

1. 日付: 2026-09-01
2. 起きたこと: 配布資料（PR #982）に載せる保険会社ポータルの検索画面を撮影しようとしたところ HTTP 500 になった。本番 DB で直接叩いて原因を特定: `insurer_accessible_tenant_ids` が `SET search_path = ''` を持ちながら本体は `FROM insurer_tenant_access` とスキーマ非修飾のままで、呼ぶと必ず `42P01 relation "insurer_tenant_access" does not exist` で落ちる。全 SECURITY DEFINER 関数を走査したところ、同じ形が `is_pii_disclosed`（`FROM pii_disclosure_consents`）にもあった。**この2本だけ。**
3. 以前の考え: `20260404000000_fix_security_definer_search_path.sql` は「SECURITY DEFINER に search_path を付けて乗っ取りを防ぐ」セキュリティ強化で、13関数に `ALTER FUNCTION ... SET search_path = ''` を当てた。関数の属性を変えるだけなので副作用は無いと考えていた。
4. 違和感・問題: (a) **`ALTER FUNCTION ... SET search_path` は本体を再検証しない。**同じ状態を `CREATE` で作ろうとすると Postgres が `check_function_bodies` で弾く —— つまりこの壊れ方は ALTER 経由でしか入り込まない。強化のつもりの変更が、本体を修飾していない関数を実行時エラーに変えていた。(b) 落ちるのは実行時なので、マイグレーションは通り、型検査も CI も素通りする。(c) 影響は1本では済まない。`insurer_search_certificates` / `insurer_search_vehicles` / `insurer_search_stores` の3本がこの関数を呼ぶので連鎖して失敗し、**保険会社ポータルの検索が丸ごと使えない状態だった。**(d) 13関数のうち他の11本は、その後の別マイグレーションで再作成されて `search_path=public, extensions` に戻っていたため生き延びていた。**偶然そうなっただけで、設計上守られていたわけではない。**(e) 発見の経路が「営業資料のスクリーンショットを撮ろうとした」というのが問題の本質を示している —— 誰も保険会社ポータルの検索を実行していなかった。
5. 決めたこと: (a) 2本を `CREATE OR REPLACE` で `public.` 修飾して直す。`SECURITY DEFINER` / `STABLE` / `search_path=''` は元のまま保ち、変えるのは本体の参照だけ。(b) **`scripts/replay-migrations.mjs` に検査を足す。**空 DB への再生後に「`search_path=''` の SECURITY DEFINER 関数が、public に実体のある名前を非修飾で参照していないか」を走査し、1件でもあれば CI を落とす。(c) その検査自体の空振りを防ぐため、**毎回わざと壊した関数を1本作って拾えることを確認してから**本走査に移る。壊し方は本番と同じ経路（作ってから ALTER）にする。
6. 捨てた選択肢: (a) `search_path` を `public, extensions` に緩める＝元のマイグレーションが防ごうとした乗っ取りの穴が戻る。**セキュリティ強化を取り消す形の修正はしない。**(b) 2本を直すだけで検査を足さない＝同じ ALTER を書けばまた入る。今回まさに「気づく手段が無かった」ことが5か月の放置を生んだ。(c) 全 SECURITY DEFINER 関数を機械的に再作成して修飾する＝影響範囲が広すぎる。壊れている2本だけ直す。
7. 判断理由: 「対症療法ではなく根本原因」「非トリバルなロジックは、壊れたら落ちる検査を1つ残す」（CLAUDE.md）。根本原因は2本の書き忘れではなく、**ALTER が本体を再検証しないという性質に対して検査が無かったこと**。CREATE では作れない形なので、人がレビューで気づくのは構造的に難しい。機械に見させるしかない。
8. まだ答えが出ていないこと: (a) いつから壊れていたか。`20260404000000` は 2026-04-04 付だが、実際に本番へ適用された日は未確認【要確認】。(b) 保険会社ポータルの利用者から問い合わせが来ていたかは未確認【要確認】。(c) **本番への反映方法。**PR をマージして `db-migrate` の CI に任せるか、Supabase MCP で直接適用するか。直接適用は本番 DB への書き込みなので代表判断待ち。(d) 同種の「実行時にしか落ちない DB オブジェクト」が他にもないか（ビュー・トリガ・RLS ポリシー）は未調査【要確認】。
9. 公開区分: 公開可（note 候補: 「セキュリティを締めた変更が、機能を静かに壊していた」。CREATE では作れない状態が ALTER でだけ作れる、という非対称が核。テナント名・利用者情報は含まない）
