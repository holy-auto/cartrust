...
| **PR-3** | `/v/[vin]` 公開ページ + `PassportTimeline` + イベント集約ロジック | M |
| **PR-4** | `/c/[public_id]` へのパスポートバッジ追加 + 管理画面からの導線 | S |
| **PR-5** | NFC書き込み先切替 + 既存タグの「パスポートURLに更新」UI | S |
| **PR-6** | opt-out トグル + 既存証明書のパスポート集約のバッチ backfill | S |
...
1. **VIN未入力の車両はパスポート対象外でよいか？**
   → 現状 `vin_code` は nullable。車検証OCRの普及状況次第では、車台番号の任意代替キー（車両番号+年式ハッシュ等）も検討余地あり。v1 は「VINあるもののみ」で割り切りたい。

2. **Polygon アンカーが画像単位**である事実をUIでどこまで見せるか？
   → 案A: 各証明書カードに「この証明書の画像3枚全てアンカー済み ✓」バッジ
   → 案B: パスポート全体として「N枚の画像がアンカーされています」だけ
   → 推奨: **案A**（信頼性の可視化が目的なので）

3. **パスポート自体を Polygon にアンカーするか？**（メタアンカー）
   → 現状は個別画像のアンカーの集合。将来、パスポート digest（VIN + 含まれる全txHashのmerkle）をPolygonに打つと「このパスポート全体が公式です」という1つの証拠に集約できる。v2候補。

4. **海外/輸入車のVIN（17文字）と国内車両の車台番号（可変長）の混在**
   → 現状 `vin_code` は可変長 text。正規化ルール (D4) はそのまま両対応可能だが、検索UX（部分一致 or 完全一致）は別途検討。
