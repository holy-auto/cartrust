// このファイルは src/types/document.ts のコピーです。編集しないでください。
// 帳票の種別・ステータス・遷移は Web と食い違うと実害が出る（モバイルで押せた
// 遷移がサーバに弾かれる等）ので、正を1つに保ったままコピーの一致を
// document.check.ts が npm test で機械的に検証します。
// 変更するときは src/types/document.ts を直し、このファイルへコピーし直すこと。
