...
-- ─────────────────────────────────────────────────────────────────────────────
-- 【後から追記】core_tables より前の日付に置かれてしまった3本の中身を、依存が
-- 揃ったこの位置で作り直す。
--
-- 経緯: `20260312000000` / `20260313000000` / `20260313000001` は、ファイル名の
-- 日付がこのファイルより前なのに、ここで作られる tenants / certificates /
-- tenant_memberships に依存していた。空 DB へ**ファイル名順に1パスで**流す
-- Supabase のブランチ機能は1本目で止まる。3本のファイル名を後ろへ動かせば順序は
-- 直るが、版番号が変わって本番で再適用になり、当時の役割を見ない RLS ポリシーや
-- search_path 未固定の関数定義が復活してしまう。そこで3本は「前提が無ければ skip」
-- に変え、実体をここへ移した。
--
-- **新しいファイルを作らないのが要点。** 本番の schema_migrations の最新より古い
-- バージョンを後から足すと `supabase db push` が out-of-order で停止し、以降の
-- マイグレーションが本番へ一切届かなくなる（.github/workflows/db-migrate.yml の
-- 不変条件2）。このファイルは適用済みなので、中身をいくら変えても再適用されない。
--
...
BEGIN
  ...
  -- CONCURRENTLY は付けない: このファイルは本番では再適用されず、空 DB では対象が空。
